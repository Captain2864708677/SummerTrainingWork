<template>
  <div>
    <div>
      ...
      <van-notice-bar left-icon="volume-o" mode="closeable">东软睿购夏季大促7.23开始！全场优惠多多，满减多多！消费金额前一百名顾客更有机会赢取幸运大奖！
      </van-notice-bar>
      ...
      <van-swipe-cell v-for="item in cartList" :key="item.id">
        <van-checkbox-group v-model="checkGroup" @change="chooseChange()" v-if="item.active === 1">
          ...
          >
            ...
            <template #tags>
              <van-tag plain type="danger" @click="toDetail(item.productId)">{{ item.detail }}</van-tag>
            </template>
            <template #footer>
        ...
        <template #right>
          <van-button square text="删除" type="danger" @click="del(item.id,0)" class="delete-button"/>
        </template>
      </van-swipe-cell>
    </div>
